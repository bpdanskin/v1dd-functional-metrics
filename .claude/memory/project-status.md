---
name: project-status
description: "Refactor P0-P6 done (2026-09-09); since then a metric overhaul shipped — events traces, greedy RF (+area, +population RF), cross-validated OSI/DSI, frac_zero pruned — regenerated as the 2026-09-14 asset and explorer V13. Wide table 110 cols (9 identity + 101 metric)."
metadata:
  type: project
  modified: 2026-09-15
---
...
## Current state (post-P6, as of 2026-09-15)

The pipeline was done and verified at P6 (below). Since then a **metric overhaul** shipped
and was regenerated by a fresh capsule run:

- **Trace types → events** for every family (RF switched events + 2-frame window); `reliability`
  primary is dF/F with an events companion (`reliability_events`). See [[fresh-start-metric-changes]],
  docs/explorations/events_vs_dff.md.
- **Greedy pixelwise RF is the default** (`rf_method="greedy"`; fraction kept behind
  `REFERENCE_CONFIG`): per-pixel STA + bootstrap + Holm-Šidák, strict + sensitive (`_a05`)
  variants, plus **RF area** columns. Archive is `rf_sta`/`rf_ge`/`strict_mask`. Retired the
  RF-area caution. docs/explorations/greedy_rf.md.
- **Cross-validated OSI/DSI** (`dg_crossval=True`; gOSI/preferred_dir stay naive) — de-biases
  the naive argmax by ~0.2 on responsive cells. docs/explorations/crossval_osi_dsi.md.
- **Population-RF** columns added to surround_suppression; **`frac_zero_response` pruned**.
- Divergence set (`differs_from_reference_config`) is now **12** settings (adds `dg_crossval`,
  `rf_method`).

Current wide table: **110 columns (9 identity + 101 metric)**. Current asset:
`results/409828_V1DD_functional_metrics_2026-09-14_22-59-50/`. Explorer rebuilt against it
and republished (V13) — see [[data-explorer-artifact]]. Local pytest: 76 pass; the only
failures are the 4 pre-existing `aind_data_schema`-not-installed ones (they pass in the capsule).
`replay_reference.py` still targets a `REFERENCE_CONFIG` asset (fraction RF, naive OSI/DSI) —
updating it for the greedy/CV default is a pending follow-up.

The P0-P6 record below is kept as history; its column counts (95) and asset date predate this
overhaul.
